...
export default function TopBar() {
  ...
  return (
    <div className="absolute top-4 left-4 right-4 z-[1000] flex items-center gap-3 pointer-events-none">
      ...
      <form onSubmit={handleSearch} className="ui-card pointer-events-auto flex items-center px-3 sm:px-4 py-2 gap-3 w-full max-w-xl md:max-w-2xl rounded-lg border border-slate-100">
        ...
        <button type="button" onClick={handleLocate} aria-label="Locate me">
          <MapPin size={18} className="text-orange-500 cursor-pointer hover:scale-110 transition" />
        </button>
      </form>
      ...
      <div className="ml-auto flex items-center gap-3 pointer-events-auto">
        <div className="hidden md:flex ui-card bg-slate-900 text-white shadow px-4 py-2 rounded-lg items-center gap-3">
          <div className="flex flex-col leading-none">
            <span className="text-[10px] text-slate-300 font-bold uppercase tracking-wider">Today's Spots</span>
        ...
        <div className="ui-card px-3 py-2 rounded-lg flex items-center gap-2">
          <label className="text-sm text-slate-600">Verified only</label>
          <button
          ...
          >
            <div className={`bg-white w-4 h-4 rounded-full shadow transform transition ${verifiedOnly ? 'translate-x-4' : ''}`}></div>
          </button>
        </div>
      </div>
    </div>
